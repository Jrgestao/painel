...
} from './services-board-core-v20.mjs?v=1'
import { aggregateImportEntries, normalizeImportText, parseImportDate, parseImportMinutes } from './services-import-core-v20.mjs?v=1'

const createClient = window.supabase?.createClient
if (typeof createClient !== 'function') {
  throw new Error(
    'Cliente local do Supabase não carregou para Serviços Executados.',
  )
}
...
async function parseCodesWorkbook(workbook) {
  ...
  if (!sheet) {
    throw new Error(
      'A planilha em códigos não possui uma aba válida.',
    )
  }
  ...
  const dateColumn = findHeaderColumn(
    headers,
    ['data'],
  )
  const serviceColumn = findHeaderColumn(
    headers,
    ['tipo_servico', 'tipo serviço'],
  )
  const timeColumn = findHeaderColumn(
    headers,
    ['horario', 'horário', 'horas'],
  )
  const observationColumn = findHeaderColumn(
    headers,
    ['observacao', 'observações', 'observações'],
  ) || 28

  if (!dateColumn || !serviceColumn || !timeColumn) {
    throw new Error(
      'A planilha não corresponde ao formato em códigos do JR Gestão.',
    )
  }
  ...
  for (
    ...
    const row = sheet.getRow(rowNumber)
    const date = importCellValue(
      row.getCell(dateColumn),
    )
    const service = String(
      importCellValue(
        row.getCell(serviceColumn),
      ) || '',
    ).trim()
    const time = String(
      importCellValue(
        row.getCell(timeColumn),
      ) || '',
    ).trim()
    ...
    const observation = String(
      importCellValue(
        row.getCell(observationColumn),
      ) || '',
    ).trim()

    const kind =
      service === '162' ||
      normalizeImportText(service).includes(
        'levantamento',
      )
        ? 'survey'
        : 'point'
    ...
    const record = {
      ...
      observation,
      products: importProductsFromCodes(
        row,
        headers,
      ),
    }
...
async function parseNormalWorkbook(workbook) {
  if (!workbook.worksheets.length) {
    throw new Error(
      'A planilha normal não possui abas.',
    )
  }
  ...
  for (
    ...
    const streetColumn = findHeaderColumn(
      headers,
      ['rua', 'logradouro'],
    )
    const orderColumn = findHeaderColumn(
      headers,
      ['ordem de servico', 'ordem_servico', 'ordem de serviço'],
    )
    const neighborhoodColumn = findHeaderColumn(
      headers,
      ['bairro'],
    )
    ...
    const timeColumn = findHeaderColumn(
      headers,
      ['horas', 'horario', 'horário'],
    ) || 5

    const observationColumn = findHeaderColumn(
      headers,
      ['observacoes', 'observações', 'observacao'],
    ) || (sheetIndex === 1 ? 8 : 28)
    ...
    // Do not interpret an unrelated XLSX as a JR report.
    if (headerMatches < 3) {
      continue
    }
    ...
    const headerDate =
      singleDateFromHeader(
        importCellValue(
          sheet.getCell('H1'),
        ),
      )

    const kind = normalSheetKind(
      sheet,
      sheetIndex,
    )
    ...
    for (
      ...
      const order = String(
        importCellValue(
          row.getCell(orderColumn || 6),
        ) || '',
      ).trim()

      if (!time && !order) continue
      ...
      const date =
        dateFromTimeText(time) ||
        headerDate

      const record = {
        serviceType: {
          code: kind === 'survey' ? '162' : '',
          name: kind === 'survey' ? 'Levantamento' : '',
        },
        observation,
        products:
          kind === 'point'
            ? importProductsFromNormal(row)
            : [],
      }
  ...
  if (!recognizedSheets) {
    throw new Error(
      'O arquivo não corresponde à planilha em códigos nem ao relatório normal do JR Gestão.',
    )
  }
...
async function downloadWorkbook() {
  try {
    ...
    downloadBlob(
      ...
      `SERVICOS_EXECUTADOS_MATRIZ_V20_${state.month}.xlsx`,
    )
