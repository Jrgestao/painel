...
function bindEvents() {
  ...
  document.addEventListener('jr:monthdata', (event) => {
    ...
    const targetMonth =
      ...
      jrCurrentMonthCampoGrandeV401613()

    // V40.16.13: estando em Servicos Executados, um cache velho da Home
    // (ex.: agosto) NAO pode trocar o mes atual escolhido pela tela.
    if (
      isServicesPageVisible() &&
      eventMonth &&
      ...
      if (
        !state.cache ||
        normalizeMonth(state.cache?.month) !== targetMonth
      ) {
        window.setTimeout(
          () => requestMonthFromMain(targetMonth, true),
          0,
        )
      }
      return
    ...
    const nextMonth = eventMonth || targetMonth
    const cacheChanged =
...
async function activateServicesPage() {
  ...
  // Se a Home ainda nao entregou o MES ATUAL, pede pelo fluxo original.
  // Nao cria consulta paralela nem substitui a matriz antiga.
  if (
    !state.cache ||
    normalizeMonth(state.cache?.month) !== targetMonth
  ) {
