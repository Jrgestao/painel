...
import {
  ...
  downloadBlob,
} from './export-rede-desempenho-v6-20260801-213442.js'
...
function showLoadingScreenV34(message = 'Preparando o painel...') {
  const screen = ensureLoadingScreenV7()
  window.clearTimeout(screen._jrHideTimerV7)
  screen.hidden = false
  screen.classList.remove('is-leaving')
  document.body.classList.add('jr-panel-loading-v7')
  updateLoadingScreenV7(message)
}
function hideLoadingScreenV34() {
  const screen = document.getElementById('jr-loading-screen-v7')
  document.body.classList.remove('jr-panel-loading-v7')
  if (!screen || screen.hidden) return

  window.clearTimeout(screen._jrHideTimerV7)
  screen.classList.add('is-leaving')
  screen._jrHideTimerV7 = window.setTimeout(() => {
    screen.hidden = true
    screen.classList.remove('is-leaving')
  }, 220)
}
...
async function loadDashboard(showSuccessToast = false, source = 'user') {
  ...
  if (automatic) {
    if (!state.autoRefresh || !state.profile || document.hidden || !navigator.onLine) return false
    if (now < state.rateLimitBackoffUntil) return false
    if (now - state.lastDashboardLoadAt < AUTO_REFRESH_MIN_GAP_MS) return false
  }

  if (state.loading) {
    if (!automatic && !initial) state.pendingDashboardRefresh = true
    return false
  }
  ...
  try {
    ...
    const rowSelect = 'id, user_id, data, registro, device_id, deleted_at, deleted_by, deleted_device_id, updated_at, sync_version'
    ...
    if (initial) {
      updateLoadingScreenV7('Buscando pontuações do dia e do mês...')
      const [monthRecords, manifests] = await Promise.all([
      ...
      dayRecords = state.monthRecords.filter((row) => serviceDateKey(row) === date)
      dayManifests = manifests
    } else {
      ;[dayRecords, dayManifests] = await Promise.all([
        fetchAllRows((from, to) => supabase
          .from('service_records')
          .select(rowSelect)
    ...
    const monthIsFresh = (
      state.monthLoadedKeyV7 === month.start &&
      Date.now() - Number(state.monthLoadedAtV7 || 0) < 900000
    )
    if (monthIsFresh) {
    ...
    scheduleIdleV7(updateMonth, 900)
    return true
  } catch (error) {
    setConnection(false)
    ...
    if (!initial && state.pendingDashboardRefresh) {
