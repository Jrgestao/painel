...
// JR_GESTAO_SUPABASE_LOCAL_SERVICOS_V40_16_9=20260902
const createClient = window.supabase?.createClient
if (typeof createClient !== 'function') {
  throw new Error('Cliente local do Supabase não carregou em Serviços Executados.')
}
...
function jrCurrentMonthCampoGrandeV401611() {
...
// JR_GESTAO_MES_ATUAL_PROTEGIDO_V40_16_13=20260902
function jrCurrentMonthCampoGrandeV401613() {
  const parts = new Intl.DateTimeFormat('en-US', {
    timeZone: 'America/Campo_Grande',
    year: 'numeric',
    month: '2-digit',
...
const state = {
  cache: null,
  profile: null,
  month: jrCurrentMonthCampoGrandeV401613(),
  settingsMonth: '',
  settings: new Map(),
  drafts: new Map(),
  selectedTeam: 'all',
...
function bindEvents() {
  ...
  document.addEventListener('jr:monthdata', (event) => {
    ...
    const eventMonth = normalizeMonth(event.detail?.month)

    // Enquanto Servicos Executados esta aberto, a Home nao pode substituir
    // a matriz fresca por cache velho/provisorio. A propria tela consulta o mes.
...
// JR_GESTAO_MES_ATUAL_SEM_CACHE_ANTIGO_V40_16_9=20260902
function jrCurrentMonthV40169() {
  return jrCurrentMonthCampoGrandeV401611()
}
// JR_GESTAO_MATRIZ_ATOMICA_PONTUACAO_FRESCA_V40_16_16=20260902
let jrServicesFreshLoadTokenV401616 = 0
let jrServicesFreshLoadPromiseV401616 = null

function jrClearMatrixRuntimeCachesV401616() {
...
function jrMonthFetchBoundsV401616(monthValue) {
  const match = String(monthValue || '').match(/^(\d{4})-(\d{2})$/)
  if (!match) throw new Error('Mes invalido para montar a matriz.')
  const year = Number(match[1])
  const monthIndex = Number(match[2]) - 1
  const first = new Date(Date.UTC(year, monthIndex, 1))
  const next = new Date(Date.UTC(year, monthIndex + 1, 1))
  return {
    start: new Date(first.getTime() - 86400000).toISOString(),
    end: new Date(next.getTime() + 86400000).toISOString(),
  }
}

async function jrFetchAllMonthRowsV401616(monthValue) {
  const bounds = jrMonthFetchBoundsV401616(monthValue)
  const rows = []
  const pageSize = 1000
  let from = 0
  ...
  return rows.filter(
    (row) => serviceDateKey(row).startsWith(monthValue + '-'),
  )
}

async function jrFetchProfilesV401616() {
  const fallback =
...
function jrPrepareAtomicMatrixLoadingV401616(monthValue) {
  const month = normalizeMonth(monthValue) || currentMonthValue()
  state.month = month
  setUiControlValue('services-month-filter', month)
  if (els.month) els.month.value = month
  clearError()
  showLoading(true)
  jrClearMatrixRuntimeCachesV401616()

  // Nao deixa a pontuacao antiga/provisoria aparecer enquanto busca o mes real.
  if (els.root) els.root.replaceChildren()
  return month
}

async function jrLoadFreshMatrixV401616(monthValue, reason = 'open') {
  const month = jrPrepareAtomicMatrixLoadingV401616(monthValue)
  const token = ++jrServicesFreshLoadTokenV401616

  const task = (async () => {
    try {
      // Duplo frame: garante que o spinner apareca antes da consulta pesada.
      await new Promise((resolve) =>
        requestAnimationFrame(() => requestAnimationFrame(resolve)),
      )

      const [records, profiles] = await Promise.all([
        jrFetchAllMonthRowsV401616(month),
        jrFetchProfilesV401616(),
      ])

      if (token !== jrServicesFreshLoadTokenV401616) return false
      ...
      const freshCache = {
        ...
        updatedAt: Date.now(),
        source: 'services-direct-v40-16-16',
      }

      state.cache = freshCache
      state.profile = freshCache.profile
      state.month = month
      jrClearMatrixRuntimeCachesV401616()

      // Atualiza o cache compartilhado somente DEPOIS de buscar TODAS as paginas.
      window.__JR_SERVICES_MONTH_CACHE__ = freshCache

      await new Promise((resolve) =>
        requestAnimationFrame(() => requestAnimationFrame(resolve)),
      )

      const ok = await loadSettingsAndRender(
        false,
        false,
        false,
      )
      ...
      console.info(
        '[JR V40.16.16] Matriz final montada:',
        month,
        records.length,
        reason,
      )
      ...
      if (token === jrServicesFreshLoadTokenV401616) {
        setError(friendlyError(error))
        console.error('[JR V40.16.16] Falha ao montar matriz fresca:', error)
      }
      return false
    } finally {
      if (token === jrServicesFreshLoadTokenV401616) {
        showLoading(false)
        jrServicesFreshLoadPromiseV401616 = null
      }
    }
  })()

  jrServicesFreshLoadPromiseV401616 = task
  return task
}
...
async function activateServicesPage() {
  ...
  // V40.16.16: nao desenha cache antigo. Busca o mes inteiro e so depois mostra a matriz.
  await jrLoadFreshMatrixV401616(month, 'open')
}
...
function syncFromMainCache() {
  ...
  const targetMonth =
    normalizeMonth(state.month) ||
    jrCurrentMonthV40169()
  const cacheMonth =
    normalizeMonth(cache.month)

  // Um cache antigo da Home nao pode trocar sozinho o mes da tela.
  if (!cacheMonth || cacheMonth !== targetMonth) return
  ...
  if (els.month) els.month.value = targetMonth
}

function requestMonthFromMain(month, force = false) {
  const target = normalizeMonth(month) || currentMonthValue()
  // Mantem o nome antigo para nao quebrar os eventos existentes, mas a carga agora
  // e direta no Supabase e nao aceita o cache mensal de 5 minutos da Home.
  void jrLoadFreshMatrixV401616(
    target,
    force ? 'refresh' : 'month-change',
  )
}
...
function loadCachedMatrixV21(_fingerprint) {
  // V40.16.16: pontuacao nunca vem de snapshot antigo.
  // A matriz e recalculada a partir dos registros frescos do Supabase.
  return null
}
...
function availableObservationDays() {
  const fromButtons = [...document.querySelectorAll('#services-board-root [data-services-view-notes]')]
    .map((node) => Number(node.getAttribute('data-services-view-notes')))
    .filter((day) => Number.isInteger(day) && day >= 1 && day <= 31)
  const unique = [...new Set(fromButtons)].sort((a, b) => a - b)
  if (unique.length) return unique
  const total = Math.max(1, Number(daysInMonth(state.month)) || 31)
  return Array.from({length: total}, (_, index) => index + 1)
}

function openObservationViewer(dayNumber) {
  const day = Number(dayNumber)
  const viewer = document.getElementById('services-observation-viewer') || els.viewer

  if (!viewer || !Number.isInteger(day) || day < 1 || day > 31) {
    try { showToast('Não foi possível abrir as observações deste dia.', true) } catch (_) {}
    return
  }

  // Atualiza referencias do dialog pelo DOM atual. Nao depende de referencia velha.
  els.viewer = viewer
  els.viewerTitle = document.getElementById('services-observation-viewer-title') || els.viewerTitle
  els.viewerPeriod = document.getElementById('services-observation-viewer-period') || els.viewerPeriod
  els.viewerDays = document.getElementById('services-observation-viewer-days') || els.viewerDays
  els.viewerBody = document.getElementById('services-observation-viewer-body') || els.viewerBody
  els.viewerPrev = document.getElementById('services-observation-viewer-prev') || els.viewerPrev
  els.viewerNext = document.getElementById('services-observation-viewer-next') || els.viewerNext
  els.viewerEdit = document.getElementById('services-observation-viewer-edit') || els.viewerEdit
  els.viewerClose = document.getElementById('services-observation-viewer-close') || els.viewerClose

  try {
    const hiddenDays = draftFor(GLOBAL_KEY).hiddenDays
    if (hiddenDays.has(day)) {
      showToast(
        `O dia ${String(day).padStart(2, '0')} está oculto. Desoculte o dia para ver suas observações.`,
      )
      return
    }

    if (!state.month) state.month = currentMonthValue()
    state.viewerDay = Math.min(
      Math.max(day, 1),
      daysInMonth(state.month),
    )

    try { hideObservationHover() } catch (_) {}
    renderObservationViewer()
    ...
    // showModal e o caminho principal. Se o navegador recusar, usa open como fallback.
    try {
      if (!viewer.open && typeof viewer.showModal === 'function') viewer.showModal()
      else if (!viewer.open) viewer.setAttribute('open', '')
    } catch (showError) {
      console.warn('[JR V40.16.20] showModal falhou; usando fallback open.', showError)
      viewer.setAttribute('open', '')
    }

    if (!viewer.hasAttribute('open')) viewer.setAttribute('open', '')
    viewer.setAttribute('aria-hidden', 'false')

    window.requestAnimationFrame(() => {
      try { document.getElementById('services-observation-viewer-close')?.focus({preventScroll:true}) } catch (_) {}
    })
  } catch (error) {
    console.error('[JR V40.16.20] Falha real ao abrir observacoes:', error)
    try {
      viewer.removeAttribute('hidden')
      viewer.classList.remove('hidden')
      viewer.setAttribute('open', '')
    ...
    try {
      showToast(
        'Erro ao abrir observações: ' + String(error?.message || error || 'erro desconhecido'),
        true,
      )
    } catch (_) {}
  }
...
// JR_GESTAO_EXCELJS_ESTAVEL_V40_16_9=20260902
...
async function ensureExcelJsForImport() {
  ...
  excelImportPromise = new Promise((resolve, reject) => {
    ...
    const succeed = () => {
      if (finished || !window.ExcelJS) return false
      finished = true
      cleanup()
    ...
    script.addEventListener('load', () => {
      if (!succeed()) {
        fail('A biblioteca de planilhas não iniciou.')
      }
    }, { once: true })

    script.addEventListener('error', () => {
      fail('Não foi possível carregar o leitor de planilhas.')
    }, { once: true })

    // Se o script ja estava carregando quando este modulo entrou,
    // esperamos ExcelJS aparecer em vez de falhar no setTimeout(0).
    ...
    timeout = window.setTimeout(() => {
      if (!succeed()) {
        fail('O gerador de planilhas demorou demais para carregar.')
      }
    }, 12000)
...
// JR_GESTAO_DESTAQUES_IMPORTADOS_UNIDOS_V40_16_9=20260902
function importedImportantForRecordV25(
  ...
  for (const text of importedImportantLinesV28(entry)) {
    ...
    if (clean) {
      result.set(
        normalizeImportText(clean),
        clean,
      )
    }
  }

  // Nao usa mais observacao OU servico. Soma os dois.
  // Assim um texto qualquer na observacao nao apaga um servico
  // importante reconhecido pelo codigo/nome do servico.
...
// JR_GESTAO_DOWNLOAD_4_XLSX_NATIVO_V40_16_9=20260902
function jrU16V40169(value) {
  const out = new Uint8Array(2)
  new DataView(out.buffer).setUint16(0, value, true)
  return out
}

function jrU32V40169(value) {
  const out = new Uint8Array(4)
  new DataView(out.buffer).setUint32(0, value >>> 0, true)
  return out
}

function jrConcatV40169(parts) {
...
function jrDosDateTimeV40169(date = new Date()) {
...
function jrZipV40169(entries) {
  ...
  for (const entry of entries) {
    ...
    const crc = jrCrc32V40169(data)
    const { dosTime, dosDate } = jrDosDateTimeV40169()

    const local = jrConcatV40169([
      jrU32V40169(0x04034b50),
      jrU16V40169(20),
      jrU16V40169(0x0800),
      jrU16V40169(0),
      jrU16V40169(dosTime),
      jrU16V40169(dosDate),
      jrU32V40169(crc),
      jrU32V40169(data.length),
      jrU32V40169(data.length),
      jrU16V40169(nameBytes.length),
      jrU16V40169(0),
      nameBytes,
      data,
    ])
    ...
    centralParts.push(jrConcatV40169([
      jrU32V40169(0x02014b50),
      jrU16V40169(20),
      jrU16V40169(20),
      jrU16V40169(0x0800),
      jrU16V40169(0),
  ...
  const local = jrConcatV40169(localParts)
  const central = jrConcatV40169(centralParts)
  const end = jrConcatV40169([
    jrU32V40169(0x06054b50),
    jrU16V40169(0),
    jrU16V40169(0),
    jrU16V40169(entries.length),
    jrU16V40169(entries.length),
    jrU32V40169(central.byteLength),
    jrU32V40169(local.byteLength),
    jrU16V40169(0),
  ])
...
function jrSafeFileV40169(value, fallback = 'PLANILHA') {
...
function jrFillMetricWorksheetV40169(workbook, metric, sheets) {
...
function jrTimeoutV40169(promise, timeoutMs, label) {
  let timer = 0
  return Promise.race([
...
function jrWaitMonthCacheV40169(month, timeoutMs = 15000) {
  if (
    normalizeMonth(state.month) === month &&
  ...
  return new Promise((resolve, reject) => {
    ...
    const onMonthData = (event) => {
      if (normalizeMonth(event.detail?.month) !== month) return
    ...
    timer = window.setTimeout(() => {
      cleanup()
      reject(new Error(`Os dados de ${month} não terminaram de carregar.`))
    }, timeoutMs)
...
// JR_GESTAO_XLSX_UNICO_4_ABAS_V40_16_14=20260902
function jrYieldUiV401614() {
...
async function downloadWorkbook() {
  setButtonLoading(els.download, true, 'Preparando planilha...')

  try {
    await jrYieldUiV401614()

    const ExcelJS = await ensureExcelJsForImport()
    if (!ExcelJS?.Workbook) {
      throw new Error('Biblioteca ExcelJS indisponível para gerar a planilha.')
    }
    ...
    // V40.16.14: UM UNICO XLSX contendo as quatro planilhas como abas.
    ...
    for (let index = 0; index < SERVICE_METRICS.length; index += 1) {
      ...
      setButtonLoading(
        els.download,
        true,
        `Montando aba ${index + 1}/${SERVICE_METRICS.length}...`,
      )
      jrFillMetricWorksheetV40169(workbook, metric, sheets)
      await jrYieldUiV401614()
    }

    if (workbook.worksheets.length !== SERVICE_METRICS.length) {
      throw new Error(
        `Foram montadas ${workbook.worksheets.length}/${SERVICE_METRICS.length} abas.`,
      )
    }

    setButtonLoading(els.download, true, 'Gerando arquivo Excel...')
    await jrYieldUiV401614()
    ...
    showToast(
      'Planilha Excel gerada com as 4 abas do mês selecionado.',
    )
  } catch (error) {
    console.error('[JR V40.16.14] Falha ao gerar XLSX único:', error)
...
function currentMonthValue() {
  return jrCurrentMonthCampoGrandeV401613()
}
...
// JR_GESTAO_OBSERVACOES_CORE_RESTAURADO_V40_16_22=20260902
function jrRefreshObservationViewerRefsV401622() {
  const pairs = [
    ['viewer', 'services-observation-viewer'],
    ['viewerTitle', 'services-observation-viewer-title'],
    ['viewerPeriod', 'services-observation-viewer-period'],
...
function jrOpenObservationViewerV401622(dayNumber) {
  const day = Number(dayNumber)
  if (!Number.isInteger(day) || day < 1 || day > 31) return
  jrRefreshObservationViewerRefsV401622()
  try {
    openObservationViewer(day)
  } catch (error) {
    console.error('[JR V40.16.22] Falha ao abrir observacoes:', error)
    try {
      showToast(
        'Não foi possível abrir as observações: ' +
...
function jrInstallObservationFallbackV401622() {
  if (document.documentElement.dataset.jrObsV401622 === '1') return
  document.documentElement.dataset.jrObsV401622 = '1'

  // Nao bloqueia o listener original. So entra DEPOIS se o dialog ainda nao abriu.
  document.addEventListener('click', (event) => {
    const target = event.target instanceof Element ? event.target : null
    const button = target?.closest?.('[data-services-view-notes]')
    if (!button || !button.closest('#services-board-root')) return
    const day = Number(button.getAttribute('data-services-view-notes'))
    if (!Number.isInteger(day)) return

    window.setTimeout(() => {
      const viewer = document.getElementById('services-observation-viewer')
      if (viewer?.open) return
      jrOpenObservationViewerV401622(day)
    }, 0)
  }, false)
}

jrInstallObservationFallbackV401622()
