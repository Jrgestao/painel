...
async function downloadWorkbook() {
  ...
  try {
    const selectedMonth =
      normalizeMonth(els.month?.value) ||
      normalizeMonth(state.month) ||
      monthFormatterV22.format(new Date())
    ...
    await jrWaitMonthCacheV40169(selectedMonth)

    await jrTimeoutV40169(
      loadSettingsAndRender(false, true, false),
      15000,
      'a matriz mensal',
    )

    const ExcelJS = await jrTimeoutV40169(
    ...
    for (
      ...
      jrFillMetricWorksheetV40169(
        workbook,
        metric,
        sheets,
      )

      const buffer = await jrTimeoutV40169(
      ...
      entries.push({
        name:
          `${jrSafeFileV40169(metric.sheetName)}_` +
          `${selectedMonth}.xlsx`,
    ...
    const zip = jrZipV40169(entries)
    const teamSuffix =
      state.selectedTeam === 'all'
        ? 'TODAS_EQUIPES'
        : jrSafeFileV40169(
            sheets[0]
              ? displayNameFor(sheets[0])
              : state.selectedTeam,
            'EQUIPE',
          )
    ...
    downloadBlob(
      zip,
      `JR_SERVICOS_EXECUTADOS_4_PLANILHAS_${selectedMonth}_${teamSuffix}.zip`,
    )

    showToast(
      'As 4 planilhas do mês selecionado foram baixadas em um único ZIP.',
    )
  } catch (error) {
    console.error(
      '[JR V40.16.9] Falha ao gerar as 4 planilhas:',
      error,
    )
