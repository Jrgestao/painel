...
// JR_GESTAO_OBSERVACOES_FUNCAO_ORIGINAL_V40_16_19=20260902
function jrInstallObservationOpenV401619() {
  if (document.documentElement.dataset.jrObsOpenV401619 === '1') return
  document.documentElement.dataset.jrObsOpenV401619 = '1'
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (view && view.closest('#services-board-root')) {
        const day = Number(view.getAttribute('data-services-view-notes'))
        if (!Number.isInteger(day) || day < 1 || day > 31) return
        event.preventDefault()
        event.stopImmediatePropagation()
        try {
          // IMPORTANTE: usa a funcao ORIGINAL do Services Executados.
          // A matriz atomica quebrou o listener antigo, nao o viewer.
          openObservationViewer(day)
        } catch (error) {
          console.error('[JR V40.16.19] Falha no openObservationViewer original:', error)
          try {
            showToast(
      ...
      if (edit && edit.closest('#services-board-root')) {
        const day = Number(edit.getAttribute('data-services-edit-notes'))
        if (!Number.isInteger(day) || day < 1 || day > 31) return
        event.preventDefault()
        event.stopImmediatePropagation()
        try {
          ...
          console.error('[JR V40.16.19] Falha no openNotesDialog original:', error)
          try {
            showToast(
              'Não foi possível editar as observações: ' +
                String(error?.message || error || 'erro desconhecido'),
              true,
            )
          } catch (_) {}
...
jrInstallObservationOpenV401619()
