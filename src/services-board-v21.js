...
import { initializeUiControls, refreshCustomSelect, setUiControlValue } from './ui-controls.js?v=20260731'
import {
  SERVICE_METRICS,
  SERVICE_METRIC_KEYS,
  daysInMonth,
...
import { aggregateImportEntries, normalizeImportText, parseImportDate, parseImportMinutes } from './services-import-core-v21.mjs?v=1'

const createClient = window.supabase?.createClient
if (typeof createClient !== 'function') throw new Error('Cliente local do Supabase não carregou para Serviços Executados.')
...
async function activateServicesPage() {
  ...
  await loadSettingsAndRender(true, warm)
}
...
async function loadSettingsAndRender(forceSettings = false, silentWarm = false) {
  ...
  if (!silentWarm && !warm) showLoading(true)
  clearError()

  try {
    if (forceSettings || state.settingsMonth !== state.month) {
...
function matrixFingerprintV21() {
  ...
  for (const row of records) {
    const stamp = String(row?.updated_at || row?.data || '')
    if (stamp > newest) newest = stamp
    versionSum += Number(row?.sync_version || 0)
  }
...
function scheduleMatrixCachePersistV21(sheets, fingerprint) {
  const snapshot = compactMatrixSnapshotV21(sheets, fingerprint)
  ...
  if (
    !isAdmin() ||
    globalDraft.dirty ||
    globalDraft.matrixCacheV21?.fingerprint === fingerprint
  ) {
    return
  }

  window.clearTimeout(state.sharedMatrixPersistTimer)
  state.sharedMatrixPersistTimer = window.setTimeout(async () => {
    try {
      if (draftFor(GLOBAL_KEY).dirty) return
      const latest = draftFor(GLOBAL_KEY)
      latest.matrixCacheV21 = snapshot
      await supabase
        .from('service_report_settings')
        .upsert(
          {
            ...
            team_key: GLOBAL_KEY,
            display_name: latest.displayName || 'Matriz',
            hidden_days: [...latest.hiddenDays].sort((a, b) => a - b),
            manual_notes: serializeReportSetting(latest),
          },
          { onConflict: 'month_key,team_key' },
        )
    } catch (_error) {
      // Cache compartilhado é otimização; nunca trava a tela.
    }
  }, 1200)
}
...
function buildTeamSheets() {
  ...
  records
    ...
    .forEach((row) => {
      ...
      const detected =
        detectImportantServices(record)
  ...
  els.team.innerHTML =
    ...
    sheets
      .map((sheet) => {
        ...
        return `<option value="${escapeHtml(sheet.key)}">${escapeHtml(displayNameFor(sheet) + suffix)}</option>`
      })
      .join('')
    ...
    ? items
        .map(
          (item) => `<article class="services-hover-team ${item.important ? 'is-important' : ''}">
            ...
            ${
              item.manualImportant && !item.automatic
  ...
  const cards = sheets.map((sheet) => {
    ...
    return `<article class="services-viewer-team ${parts.hasImportant ? 'is-important' : ''} ${parts.text ? '' : 'is-empty'}">
      ...
      <div class="services-viewer-team-content">
        ...
        ${
          parts.manualImportant && !parts.automatic
...
function draftFor(teamKey) {
  ...
  const draft = {
    ...
    importedScoresByMetric: JSON.parse(
      JSON.stringify(
        setting.importedScoresByMetric || {},
      ),
    ),
    importedNotesByMetric: JSON.parse(
      JSON.stringify(
...
async function parseCodesWorkbook(workbook) {
  ...
  const observationColumn = findHeaderColumn(headers, ['observacao', 'observações', 'observacoes']) || 28
  ...
  for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber += 1) {
    ...
    entries.push({
      ...
      important: detectImportantServices(record),
      sourceRow: rowNumber,
    })
...
async function parseNormalWorkbook(workbook) {
  ...
  for (let sheetIndex = 0; sheetIndex < workbook.worksheets.length; sheetIndex += 1) {
    ...
    const observationColumn = findHeaderColumn(headers, ['observacoes', 'observações', 'observacao']) || (sheetIndex === 1 ? 8 : 28)
    ...
    for (let rowNumber = 3; rowNumber <= sheet.rowCount; rowNumber += 1) {
      ...
      entries.push({
        ...
        neighborhoodName,
        orderNumber,
        observation: kind === 'point' ? observation : '',
        surveyObservation: kind === 'survey' ? observation : '',
        products: products.map((item) => ({
          code: item.product?.code || '',
          name: item.product?.name || '',
...
function renderImportPreview() {
  ...
  els.importSummary.innerHTML = `
    ...
    <div class="services-import-detail-line">
      ...
      <span><b>Serviços importantes:</b> ${importantCount}</span>
  ...
  if (els.importStatus) {
    els.importStatus.textContent = ready
      ? 'Conferência pronta. A importação substituirá somente a pontuação importada dos dias encontrados.'
      : !teamKey
...
async function applyImportedWorkbook() {
  ...
  const importedDates = new Set(
    aggregate.days.map((day) => day.date),
  )

  draft.importedRows = (draft.importedRows || []).filter(
    (item) => !importedDates.has(String(item?.date || '').slice(0, 10)),
  )
  ...
  for (const entry of state.importEntries || []) {
    ...
    if (!date || !date.startsWith(`${state.month}-`)) continue
    ...
    SERVICE_METRIC_KEYS.forEach((metric) => {
      ...
      if (important.length) {
        draft.importedNotesByMetric[metric][dayKey] =
          `IMPORTADO: ${important.join(' • ')}`.slice(0, 1600)
      } else {
  ...
  try {
    ...
    showToast(
      `Planilha importada: ${aggregate.total} item(ns) em ${aggregate.days.length} dia(s).`,
    )
  } catch (error) {
    showToast(
      friendlyError(error),
...
async function downloadWorkbook() {
  try {
    ...
    downloadBlob(
      ...
      `SERVICOS_EXECUTADOS_MATRIZ_V21_${state.month}.xlsx`,
    )

    showToast(
      'Planilha V14 com quatro abas gerada.',
    )
  } catch (error) {
    showToast(
      friendlyError(error),
...
function serviceDateKey(row) {
  ...
  // 1) O nome original da foto é a fonte mais segura para o dia real.
  for (const value of [
    ...
    const match = text.match(/(?:^|\D)(\d{4})(\d{2})(\d{2})[_\-\s](?:[01]\d|2[0-3])(?:[0-5]\d)/)
    if (match) return `${match[1]}-${match[2]}-${match[3]}`
  }

  // 2) Data real da foto convertida para Campo Grande.
  for (const value of [
  ...
  // 3) Fallback para o dia declarado pelo registro.
  for (const value of [
    record.date,
    record.serviceDate,
    record.workDate,
