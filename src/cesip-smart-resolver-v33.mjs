/*
  JR GESTAO - CESIP SMART RESOLVER V33 — LETRAS PRIMEIRO + SEMANTICO + FONETICO SONORO
  ...
  (letras escritas), depois nucleo semantico e fonetica. V33 acrescenta uma
  assinatura sonora por consoantes para erros como RAMEZ TABET -> RAMES TEBIT.
  O MENOR codigo CESIP
...
export function phoneticCesipSkeletonV33(value) {
  return phoneticSkeletonV33(value)
}
...
/*
  V33: assinatura sonora conservadora. Ela NAO substitui a grafia: serve como
  evidencia extra quando as palavras continuam com a mesma estrutura falada.
  Vogais internas variam bastante em nomes digitados de ouvido (TABET/TEBIT),
  e S/Z podem representar praticamente o mesmo som (RAMEZ/RAMES).
*/
function phoneticSkeletonV33(value) {
  const base = phoneticTokenV32(value)
  if (!base || /^\d+$/.test(base)) return base
  const voiced = base
    .replace(/Z/g, 'S')
    .replace(/(.)\1+/g, '$1')
  if (voiced.length < 4) return voiced
  const skeleton = (voiced[0] + voiced.slice(1).replace(/[AEIOU]/g, ''))
    .replace(/(.)\1+/g, '$1')
  return skeleton.length >= 2 ? skeleton : voiced
}

function phoneticSkeletonComparableV33(tokens) {
  return tokens.map(phoneticSkeletonV33).filter(Boolean).join(' ')
}

function strongSkeletonTokensV33(tokens) {
  return tokens.length >= 2 && tokens.every((token) => phoneticSkeletonV33(token).length >= 3)
}
...
function buildCatalog(items, kind) {
  ...
  const byPhoneticSkeletonNormalized = new Map()
  const byToken = new Map()
  const byPhoneticToken = new Map()
  const byPhoneticSkeletonToken = new Map()
  ...
  for (const item of Array.isArray(items) ? items : []) {
    ...
    const phoneticSkeletonTokens = tokens.map(phoneticSkeletonV33)
    const phoneticSkeletonNormalized = phoneticSkeletonTokens.join(' ')
    ...
    const entry = {
      ...
      phoneticSkeletonTokens, phoneticSkeletonNormalized,
      neighborhoodSplit,
    ...
    add(byPhoneticSkeletonNormalized, phoneticSkeletonNormalized, entry)
    ...
    for (let i = 0; i < tokens.length; i += 1) {
      ...
      const stoken = phoneticSkeletonTokens[i]
      add(byToken, token, entry)
      add(byPhoneticToken, ptoken, entry)
      add(byPhoneticSkeletonToken, stoken, entry)
  ...
  return { kind, entries, byNormalized, byPhoneticNormalized, byPhoneticSkeletonNormalized, byToken, byPhoneticToken, byPhoneticSkeletonToken, bySeed, byGram, byFirstLength, byNeighborhoodBase }
}
...
function tokenSimilarity(a, b) {
  ...
  const sa = phoneticSkeletonV33(a)
  const sb = phoneticSkeletonV33(b)
  if (sa && sb && sa.length >= 3 && sb.length >= 3 && sa === sb) {
    // Mantem abaixo de uma falta de uma letra (0.99), respeitando LETRAS PRIMEIRO.
    best = Math.max(best, 0.945)
  }
  return best
}
...
function candidatePool(catalog, sourceTokens) {
  ...
  for (const token of sourceTokens) {
    ...
    const stoken = phoneticSkeletonV33(token)
    ...
    if (stoken.length >= 3) {
      for (const entry of catalog.byPhoneticSkeletonToken.get(stoken) || []) bump(entry, 86)
    }
...
function scoreCandidate(source, candidate, kind) {
  ...
  const phoneticSkeletonWhole = wholeSimilarity(source.phoneticSkeletonNormalized, candidate.phoneticSkeletonNormalized)
  const skeletonExactStrong = strongSkeletonTokensV33(source.tokens) &&
    strongSkeletonTokensV33(candidate.tokens) &&
    source.phoneticSkeletonNormalized === candidate.phoneticSkeletonNormalized
  ...
  if (skeletonExactStrong) score += 0.11
...
function candidateStrongEnoughV32(candidate, source, kind, thresholds) {
  ...
  const evidence =
    ...
    (candidate.skeletonExactStrong && candidate.phoneticSkeletonWhole >= 0.96) ||
...
function resolveAgainstCatalog(raw, catalog, kind, options = {}) {
  ...
  const source = {
    ...
    phoneticSkeletonNormalized: phoneticSkeletonComparableV33(sourceTokens),
  ...
  const skeletonEntries = strongSkeletonTokensV33(sourceTokens)
    ? catalog.byPhoneticSkeletonNormalized.get(source.phoneticSkeletonNormalized) || []
    : []
  ...
  let candidates = exactEntries.length
    ? exactEntries
    : [...new Set([...phoneticEntries, ...skeletonEntries, ...candidatePool(catalog, sourceTokens)])].slice(0, 112)
  ...
  return {
    ...
    phoneticSkeletonWhole: chosen.phoneticSkeletonWhole,
    soundAwareV33: true,
