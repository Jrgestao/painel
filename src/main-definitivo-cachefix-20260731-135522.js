...
import { SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, INTERNAL_LOGIN_DOMAIN, APP_TIME_ZONE, ADMIN_API_BASE_URL } from './config-cachefix-20260731-135522.js'
...
import {
  ...
  downloadBlob,
} from './export-service-data-codigos-v13.js?v=20260731'
...
const AUTO_REFRESH_BASE_MS = 60000
const AUTO_REFRESH_JITTER_MS = 15000
...
function showLoadingScreenV34() {}

function hideLoadingScreenV34() {}
...
installMobileCalendarAndTeamsV37()
installServiceDateCalendarV38()
...
async function finishAuthorizedPanelLoadV32(flowVersion) {
  try {
    ...
    if (flowVersion === authFlowVersionV32 && state.profile) {
...
function installServiceDateCalendarV38() {
  ...
  if (!serviceDateObserverV38) {
    serviceDateObserverV38 = new MutationObserver((mutations) => {
      if (!isMobileLayoutV37()) return

      bindServiceDateTriggerV38()

      const addedCalendar = mutations.some((mutation) =>
        [...mutation.addedNodes].some((node) =>
          node instanceof Element &&
          (
            node.matches?.(
              '.picker-popover, .date-picker-popover, [data-picker-popover]'
            ) ||
            node.querySelector?.(
              '.picker-popover, .date-picker-popover, [data-picker-popover]'
            )
      ...
      const { popover } = getServiceDatePartsV38()
      if (addedCalendar || isServiceDatePopoverOpenV38(popover)) {
        scheduleServiceDateCalendarV38()
      }
    })

    serviceDateObserverV38.observe(document.body, {
      childList: true,
      subtree: true,
      attributes: true,
  ...
  window.addEventListener(
    ...
    () => {
      window.setTimeout(() => {
        serviceDateTriggerRectV38 = null
        positionServiceDateCalendarV38()
      }, 180)
    },
    { passive: true }
...
async function authorizeAndOpen(user, flowVersion = authFlowVersionV32) {
  ...
  void finishAuthorizedPanelLoadV32(flowVersion)
}
...
async function loadDashboard(showSuccessToast = false, source = 'user') {
  ...
  try {
    ...
    const dateInsideMonth = date >= month.start && date < month.next

    const monthFetchStart = `${addDays(month.start, -1)}T00:00:00.000Z`
    const monthFetchEnd = `${addDays(month.next, 1)}T00:00:00.000Z`

    // O mês já contém o dia selecionado na maioria dos casos. A versão antiga
    // buscava mês e dia separadamente, duplicando várias solicitações a cada
    // atualização. Agora o dia é derivado do próprio resultado mensal.
    const [monthRecords, monthManifests] = await Promise.all([
    ...
    if (
      ...
      month.start === currentOrdersMonth.start
    ) {
      state.ordersRange = {
    ...
    if (dateInsideMonth) {
      state.records = state.monthRecords.filter((row) => serviceDateKey(row) === date)
      state.manifests = state.monthManifests.filter((manifest) => String(manifest.work_date || '').slice(0, 10) === date)
    } else {
    ...
    if (!state.range.loadedKey || state.range.loadedKey === `${date}|${date}`) {
      state.range = { start: date, end: date, records: [...state.records], manifests: [...state.manifests], loadedKey: `${date}|${date}` }
...
function setRangeLoading(value) {
  ;[els.photosRefreshButton, els.codesRefreshButton, els.reportsRefreshButton, els.photosAllDownload].forEach((button) => { if (button) button.disabled = value })
}
...
function renderDashboard(options = {}) {
  ...
  const monthRecords = team === 'all' ? state.monthRecords : recordsForTeam(team, profileMap, state.monthRecords)
  const monthManifests = team === 'all' ? state.monthManifests : manifestsForTeam(team, profileMap, state.monthManifests)
...
function renderModulePages() {
  ...
  renderCurrentModuleV31('photos', true)
  renderCurrentModuleV31('codes', true)
  renderCurrentModuleV31('reports', true)
  renderCurrentModuleV31('orders', true)
}
...
async function downloadPhotosZipForRange(team = 'all') {
  if (!canDownloadPhotos()) { toast('Acesso negado. Este usuário não pode baixar fotos.', true); return }
  try {
    const { start, end } = await ensureExportRangeReady('photos')
    validateRange(start, end)
    const teamLabelValue = team === 'all' ? 'todas as equipes' : team
    toast(`Preparando ZIP de ${teamLabelValue}. O download começará automaticamente.`)
    const result = await adminApiRequest('/api/admin/photo-exports', {
      method: 'POST',
      body: { start, end, team },
    })
    if (!result?.downloadUrl) throw new Error('O servidor não retornou o endereço do ZIP.')
    ...
    link.click()
    link.remove()
    toast(`${result.photoCount || 0} foto(s) organizadas por equipe e dia. Download iniciado.`)
  } catch (error) { toast(friendlyError(error), true) }
}
...
function switchPage(page, silent = false) {
  ...
  if (!samePage) {
    scheduleCurrentModuleRenderV31(target)
  }
}
