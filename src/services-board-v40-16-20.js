...
// JR_GESTAO_OBSERVACOES_CLIQUE_REAL_V40_16_20=20260902
function jrInstallObservationClickRealV401620() {
  if (document.documentElement.dataset.jrObsClickV401620 === '1') return
  document.documentElement.dataset.jrObsClickV401620 = '1'

  let style = document.getElementById('jr-observation-click-v401620')
  if (!style) {
    style = document.createElement('style')
    style.id = 'jr-observation-click-v401620'
    document.head.appendChild(style)
  }
  style.textContent = `
    #services-board-root [data-services-view-notes],
    #services-board-root .services-observation-open {
  ...
  // Reforco SEM preventDefault/stopPropagation/stopImmediatePropagation.
  // O listener original do Services Executados continua recebendo o mesmo clique.
  document.addEventListener('click', (event) => {
    ...
    const day = Number(button.dataset.servicesViewNotes)
    if (!Number.isInteger(day)) return
    openObservationViewer(day)
  }, true)
}

jrInstallObservationClickRealV401620()
