...
function showLoadingScreenV34(message = 'Conectando ao painel...') {
  let screen = document.getElementById('jr-safe-loading-v84')

  if (!screen) {
    screen = document.createElement('div')
    screen.id = 'jr-safe-loading-v84'
    screen.className = 'jr-safe-loading-v84'
    screen.innerHTML = `
      <div class="jr-safe-loading-card-v84">
        <img src="./assets/logo-metal-liquido-v24.png" alt="JR Gestão">
        <strong>JR GESTÃO</strong>
        <p id="jr-safe-loading-message-v84">Conectando ao painel...</p>
        <div class="jr-safe-loading-track-v84"><span></span></div>
      </div>
    `
    document.body.appendChild(screen)
  }

  const messageNode = document.getElementById('jr-safe-loading-message-v84')
  if (messageNode) messageNode.textContent = message
  screen.classList.add('show')

  window.clearTimeout(safeLoadingTimerV84)
  safeLoadingTimerV84 = window.setTimeout(() => {
...
function hideLoadingScreenV34() {
  ...
  const screen = document.getElementById('jr-safe-loading-v84')
  if (!screen) return
...
async function adminApiRequest(path, options = {}) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new Error(
        'O servidor administrativo demorou mais de 15 segundos. A operação foi cancelada sem recarregar o painel.'
      )
    }
