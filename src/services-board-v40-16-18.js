...
// JR_GESTAO_OBSERVACOES_REPARO_DELEGADO_V40_16_18=20260902
function jrOpenObservationViewerSafeV401618(dayNumber) {
  const day = Number(dayNumber)
  const viewer = document.getElementById('services-observation-viewer')
  if (!viewer || !Number.isInteger(day)) return false
  ...
  try {
    ...
    requestAnimationFrame(() => {
      try {
        if (typeof jrNormalizeObservationViewerV4147 === 'function') {
          jrNormalizeObservationViewerV4147()
        }
      } catch (_) {}
    })
    ...
    console.error('[JR V40.16.18] Falha ao abrir observacoes:', error)
    try {
      viewer.classList.remove('hidden')
      viewer.setAttribute('open', '')
      viewer.setAttribute('aria-hidden', 'false')
...
function jrOpenNotesEditorSafeV401618(dayNumber) {
  const day = Number(dayNumber)
  ...
  try {
    ...
    console.error('[JR V40.16.18] Falha ao abrir editor de observacoes:', error)
    showToast(
      'Não foi possível abrir o editor de observações: ' +
        String(error?.message || error || 'erro desconhecido'),
      true,
    )
    return false
...
function jrInstallObservationDelegatedOpenV401618() {
  if (document.documentElement.dataset.jrObsOpenV401618 === '1') return
  document.documentElement.dataset.jrObsOpenV401618 = '1'

  // Listener no DOCUMENTO: continua vivo mesmo quando a matriz atomica
  // apaga e recria todo o conteudo de #services-board-root.
  document.addEventListener(
    'click',
    (event) => {
      const view = event.target?.closest?.('[data-services-view-notes]')
      if (view && view.closest('#services-board-root')) {
        event.preventDefault()
        event.stopImmediatePropagation()
        jrOpenObservationViewerSafeV401618(
          view.dataset.servicesViewNotes,
        )
        return
      }

      const edit = event.target?.closest?.('[data-services-edit-notes]')
      if (edit && edit.closest('#services-board-root')) {
        event.preventDefault()
        event.stopImmediatePropagation()
        jrOpenNotesEditorSafeV401618(
          edit.dataset.servicesEditNotes,
        )
      }
    },
    true,
  ...
  // Fechamento defensivo: evita dialog preso caso listeners antigos
  // tenham sido perdidos em alguma troca de versao.
  document.addEventListener(
    'click',
    (event) => {
      const close = event.target?.closest?.('#services-observation-viewer-close')
      if (!close) return
      event.preventDefault()
      try { closeObservationViewer() }
      catch (_) {
        const viewer = document.getElementById('services-observation-viewer')
        try { viewer?.close?.() } catch (_) { viewer?.removeAttribute('open') }
      }
    },
    true,
  )
...
jrInstallObservationDelegatedOpenV401618()
