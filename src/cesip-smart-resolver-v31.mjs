/*
  JR GESTAO - CESIP SMART RESOLVER V31 — SEMANTICO + FONETICO

  Resolve nomes imperfeitos de logradouros/bairros contra a tabela oficial
  que o app JR Gestao usa. Combina nucleo semantico, escrita e fonetica.
  Quando dois candidatos continuam realmente equivalentes depois dessas
  camadas, a regra operacional do JR Gestao e escolher o MENOR codigo CESIP.
*/
...
export function phoneticCesipTokenV31(value) {
  return phoneticTokenV31(value)
}
...
function normalizeBase(value, { kind = 'street' } = {}) {
  ...
  if (kind === 'neighborhood') {
    ...
    // Prefixos administrativos nao definem o nucleo do nome. Remove varios em cadeia:
  ...
  tokens = tokens.map((token, index) => {
    ...
    return kind === 'neighborhood' ? singularNeighborhoodToken(token) : token
  })
...
function phoneticTokenV31(value) {
...
function phoneticComparableV31(tokens) {
  return tokens.map(phoneticTokenV31).filter(Boolean).join(' ')
}
...
function buildCatalog(items, kind) {
  ...
  for (const item of Array.isArray(items) ? items : []) {
    ...
    const phoneticTokens = tokens.map(phoneticTokenV31)
  ...
  return { kind, entries, byNormalized, byPhoneticNormalized, byToken, byPhoneticToken, bySeed, byGram, byFirstLength }
}
...
function genderVariantConflictV31(a, b) {
...
function tokenSimilarity(a, b) {
  if (a === b) return 1
  if (!a || !b) return 0
  // Nomes como UNIVERSITARIA e UNIVERSITARIO sao entidades diferentes no CESIP.
  // A semelhanca ortografica/fonetica nao pode apagar essa diferenca semantica.
  if (genderVariantConflictV31(a, b)) return 0
  ...
  if (shorter.length >= 4 && longer.startsWith(shorter)) {
    ...
    if (difference <= 2) best = difference === 1 ? 0.94 : 0.9
  }

  const maxDistance = maxTokenDistance(a, b)
  if (maxDistance) {
    const distance = levenshtein(a, b, maxDistance)
    if (distance <= maxDistance) best = Math.max(best, 1 - (distance / Math.max(a.length, b.length)))
  }

  const pa = phoneticTokenV31(a)
  const pb = phoneticTokenV31(b)
  if (pa && pa === pb) best = Math.max(best, 0.985)
  else if (pa && pb && pa[0] === pb[0]) {
    const pd = levenshtein(pa, pb, 2)
    if (pd <= 2) best = Math.max(best, (1 - pd / Math.max(pa.length, pb.length)) * 0.96)
  }
  return best
}
...
function matchTokenSets(sourceTokens, targetTokens) {
  ...
  for (let si = 0; si < sourceTokens.length; si += 1) {
    for (let ti = 0; ti < targetTokens.length; ti += 1) {
      const similarity = tokenSimilarity(sourceTokens[si], targetTokens[ti])
      if (similarity >= 0.72) pairs.push({ si, ti, similarity })
    }
  }
...
function candidatePool(catalog, sourceTokens) {
  ...
  for (const token of sourceTokens) {
    const ptoken = phoneticTokenV31(token)
...
function scoreCandidate(source, candidate, kind) {
  ...
  let score =
    tokenMatch.sourceCoverage * 0.46 +
    tokenMatch.targetCoverage * 0.27 +
    whole * 0.12 +
    phoneticWhole * 0.10 +
    tokenMatch.orderScore * 0.05

  if (source.normalized === candidate.normalized) score += 0.35
  if (tokenMatch.exactCount >= Math.min(2, source.tokens.length)) score += 0.025
...
function lowestCodeEntryV31(entries) {
...
function pickExactCoreV31(entries, sourceType, kind) {
  if (!entries.length) return null
  if (entries.length === 1) return entries[0]
  if (sourceType && sourceType !== 'GENERIC') {
    const sameType = entries.filter((entry) => entry.type === sourceType)
    if (sameType.length) return lowestCodeEntryV31(sameType)
  }
  if (kind === 'neighborhood') {
    ...
    if (baseBairro.length) return lowestCodeEntryV31(baseBairro)
  }
  // Regra operacional: mesmo nucleo/tipo e mais de um codigo => menor codigo CESIP.
  return lowestCodeEntryV31(entries)
}

const NEIGHBORHOOD_DETAIL_SUFFIX_V31 = new Set([
  'SEC', 'SECAO', 'SETOR', 'QUADRA', 'QD',
  'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H',
  'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X',
])

function isNeighborhoodCoreExpansionV31(sourceTokens, targetTokens) {
  if (!sourceTokens.length || targetTokens.length <= sourceTokens.length) return false
  // O nucleo digitado precisa aparecer exatamente e na mesma ordem.
  let cursor = 0
  ...
  const extras = targetTokens.filter((token) => !sourceSet.has(token))
  if (!extras.length) return false
  return extras.every((token) => NEIGHBORHOOD_DETAIL_SUFFIX_V31.has(token) || /^\d{1,3}[A-Z]?$/.test(token))
}

function candidateStrongEnoughV31(candidate, source, kind, thresholds) {
  const coreExpansion = kind === 'neighborhood' &&
    isNeighborhoodCoreExpansionV31(source.tokens, candidate.entry.tokens)
  const evidence =
    candidate.exactCount >= 1 ||
    candidate.whole >= 0.86 ||
    candidate.phoneticWhole >= 0.9 ||
    coreExpansion
  const targetOk = candidate.targetCoverage >= thresholds.minimumTargetCoverage ||
    (coreExpansion && candidate.sourceCoverage >= 0.96)
  return candidate.score >= thresholds.minimumScore &&
    candidate.sourceCoverage >= thresholds.minimumSourceCoverage &&
    targetOk && evidence
}

function chooseLowerCodeTieV31(scored, source, kind, thresholds) {
  if (!scored.length) return null
  const top = scored[0]
  const strong = scored.filter((candidate) =>
    top.score - candidate.score <= thresholds.minimumGap &&
    candidateStrongEnoughV31(candidate, source, kind, thresholds)
  )
  if (strong.length < 2) return null
  return strong.slice().sort((a, b) =>
    a.entry.numericCode - b.entry.numericCode || b.score - a.score || a.entry.name.localeCompare(b.entry.name, 'pt-BR')
  )[0]
}
...
function resolveAgainstCatalog(raw, catalog, kind, options = {}) {
  ...
  const source = {
    ...
    phoneticNormalized: phoneticComparableV31(sourceTokens),
  ...
  const exactPick = pickExactCoreV31(exactEntries, source.type, kind)
  if (exactPick) {
    return {
  ...
  const scored = candidates
    ...
    .filter(Boolean)
    .sort((a, b) => b.score - a.score || a.entry.numericCode - b.entry.numericCode || a.entry.name.localeCompare(b.entry.name, 'pt-BR'))
  ...
  const bestStrong = exactUnique || candidateStrongEnoughV31(best, source, kind, thresholds)
  let chosen = best
  let tieBreakLowerCode = false

  if (bestStrong && gap < minimumGap) {
    const lowerCodeTie = chooseLowerCodeTieV31(scored, source, kind, thresholds)
    if (lowerCodeTie) {
      chosen = lowerCodeTie
...
export async function canonicalizeCodesRecordsV30(records, options = {}) {
  ...
  if (typeof window !== 'undefined') window.__JR_CESIP_SMART_STATS_V31__ = { ...stats, updatedAt: Date.now() }
  return result
}
