import * as JR_CONFIG from './config.js?v=admin-switch-v6-20260904'

const MARKER_ID = 'jr-admin-mode-entry-v6'
...
async function installAdminEntry() {
  ...
  style.textContent = `
    .jr-admin-mode-entry-v6{box-sizing:border-box!important;text-decoration:none!important;cursor:pointer!important}
    .sidebar-nav .jr-admin-mode-entry-v6{position:relative;margin-top:10px;border:1px solid rgba(255,255,255,.18)!important;background:linear-gradient(135deg,rgba(255,255,255,.13),rgba(255,255,255,.045))!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.12),0 10px 28px rgba(0,0,0,.18)!important;color:#f4f4f4!important}
    .sidebar-nav .jr-admin-mode-entry-v6:hover{border-color:rgba(255,255,255,.38)!important;background:linear-gradient(135deg,rgba(255,255,255,.2),rgba(255,255,255,.07))!important;transform:translateY(-1px)}
  ...
  if (nav) {
    link.className = 'nav-item jr-admin-mode-entry-v6'
    link.innerHTML = '<span class="jr-admin-mode-badge" aria-hidden="true">ADM</span><span>Administrativo</span>'
    nav.appendChild(link)
  } else {
    link.className = 'jr-admin-mode-entry-v6 jr-admin-mode-floating'
