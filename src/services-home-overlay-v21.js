...
function applyTeamCards() {
  ...
  document.querySelectorAll('.team-score-row-v30').forEach((row) => {
    ...
    if (!team || !Number.isInteger(day)) return

    const score = team.days.get(day) || blankScore()
    ...
    values.forEach((node, index) => {
      if (index < ordered.length && node.textContent !== String(ordered[index])) {
        node.textContent = String(ordered[index])
      }
    })
